...
def main():
    ...
    if st.session_state.selected:
        ...
        if st.session_state.prediction == "Covid":
            st.error(' The model indicates that the patient has Covid-19, please proceed to the hospital.Quarantine and stay at home. <a href = "https://www.mohfw.gov.in/pdf/RevisedIllustratedGuidelinesforHomeIsolationofMildAsymptomaticCOVID19Cases.pdf">Know more🔗</a>',unsafe_allow_html=True)
        if st.session_state.prediction == "Viral Pneumonia":
            st.error('The model indicates that the patient has Viral Pneumonia.It is not life threatening and is usually treatable at home,however if you have more serious symptoms,please consult a doctor.<a href = "https://www.webmd.com/lung/viral-pneumonia">Know more🔗</a>',unsafe_allow_html=True)
